...
class MemberSearchViewModel @Inject constructor (
        ...
        Completable.fromAction {
            ...
            if (query.contains(Regex("[0-9]"))) {
                ...
                observable.postValue(observable.value?.copy(
                    ...
                    loading = false,
                    searchMethod = SearchMethod.SEARCH_CARD_ID
                ))
            } else {
                ...
                observable.postValue(observable.value?.copy(
                    matchingMembers = matchingMembers,
                    loading = false,
                    searchMethod = SearchMethod.SEARCH_NAME
                ))
            }
        }.doOnError {
            logger.error(it)
    ...
    enum class SearchMethod { SEARCH_NAME, SEARCH_CARD_ID }

    data class ViewState(
        val matchingMembers: List<MemberWithThumbnail> = emptyList(),
