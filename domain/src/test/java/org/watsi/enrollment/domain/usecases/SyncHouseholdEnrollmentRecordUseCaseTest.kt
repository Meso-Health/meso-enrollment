...
class SyncHouseholdEnrollmentRecordUseCaseTest {

    @Mock lateinit var householdEnrollmentRecordRepo: HouseholdEnrollmentRecordRepository
    @Mock lateinit var deltaRepo: DeltaRepository
    lateinit var useCase: SyncHouseholdEnrollmentRecordUseCase
    ...
    fun setup() {
        ...
        useCase = SyncHouseholdEnrollmentRecordUseCase(householdEnrollmentRecordRepo, deltaRepo)
        whenever(onErrorCallback.invoke(any())).thenReturn(true)
        whenever(deltaRepo.unsynced(Delta.ModelName.HOUSEHOLD_ENROLLMENT_RECORD)).thenReturn(unsyncedSingle)
        whenever(deltaRepo.markAsSynced(deltaList)).thenReturn(Completable.complete())
    }

    @Test
    fun execute_success() {
        whenever(householdEnrollmentRecordRepo.sync(deltaList)).thenReturn(Completable.complete())
        useCase.execute(onErrorCallback).test().assertComplete()
        verify(onErrorCallback, never()).invoke(any())
        verify(householdEnrollmentRecordRepo, times(1)).sync(deltaList)
        verify(deltaRepo, times(1)).markAsSynced(deltaList)
    }

    @Test
    fun execute_onError() {
        whenever(householdEnrollmentRecordRepo.sync(deltaList)).thenReturn(Completable.error(exception))
        useCase.execute(onErrorCallback).test().assertComplete()
